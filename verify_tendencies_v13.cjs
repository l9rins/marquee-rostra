...
let lebronMod = 139378 % 911; 

console.log("Scanning exclusively for records aligned to LeBron's modulo: " + lebronMod);

let count = 0;
for (let i = 0; i < buffer.length - 1000; i++) {
    if (i % 911 === lebronMod) {
        let cfid = buffer.readUInt16LE(i + 28);
        if (cfid === 1015) dumpTendencies(i, "Dwyane Wade (1015)");
        if (cfid === 633) dumpTendencies(i, "Ray Allen (633)");
        if (cfid === 1611) dumpTendencies(i, "Stephen Curry (1611)");
        if (cfid === 1013) dumpTendencies(i, "LeBron James (1013)");
        if (cfid === 1014) dumpTendencies(i, "Chris Bosh (1014)");
        if (cfid === 1015 || cfid === 633 || cfid === 1611 || cfid === 1013 || cfid === 1014) {
            count++;
        }
    }
}
